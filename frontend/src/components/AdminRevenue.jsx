...
export default function AdminRevenue() {
  ...
  return (
    <div style={wrap}>
      ...
      <div style={{ background: 'var(--primary-tint)', borderRadius: 12, padding: '14px 16px', marginBottom: 12 }}>
        <p style={{ margin: 0, fontSize: 11.5, color: 'var(--text-2)' }}>총 수수료</p>
        <p style={{ margin: '2px 0 0', fontSize: 26, fontWeight: 700, color: 'var(--text-strong)', letterSpacing: '-0.5px' }}>{won(data.total_commission)}</p>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
          {places.map(([k, v]) => (
            <div key={k} style={{ flex: '1 1 45%', minWidth: 120, border: '1px solid var(--bd)', borderRadius: 10, padding: '10px 12px', background: 'var(--surface)' }}>
              <p style={{ margin: 0, fontSize: 12, fontWeight: 600, color: 'var(--text-2)' }}>{PLACE_LABEL[k] || k}</p>
