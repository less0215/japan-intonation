...
function PrivacyPolicy() {
  return (
    <article>
      ...
      <Section title="제4조의2 브라우저 확장 프로그램(넷플릭스 쉐도잉)">
        <p>틱재팬은 넷플릭스에서 일본어를 학습할 수 있는 크롬 확장 프로그램 「넷플릭스 일본어 자막 – 후리가나·이중 자막·쉐도잉」을 제공합니다. 본 확장의 데이터 처리는 아래와 같습니다.</p>
        <Table
          headers={['처리 데이터', '목적 및 방식']}
          rows={[
            ['자막 텍스트', '재생 중인 넷플릭스 콘텐츠의 일본어 자막을 번역·후리가나 생성을 위해 서버로 전송합니다. 생성된 학습 자막(일본어·후리가나)은 콘텐츠 단위로 저장되어 다른 학습자와 공유되며, 개인 이용자와 연결되지 않습니다.'],
            ['탭 오디오(선택)', '일본어 자막이 없는 작품에서 이용자가 직접 확장 아이콘을 눌러 요청한 경우에만 재생 오디오를 캡처하여 대사를 텍스트로 변환합니다. 오디오 자체는 저장하지 않으며 변환된 텍스트만 위와 같이 처리됩니다.'],
            ['틱재팬 계정 정보', '단어·문장 저장 기능 이용 시, 틱재팬에 로그인한 계정 식별자(회원 ID·이름)를 사용해 저장 항목을 계정에 동기화합니다. 로그인하지 않으면 수집하지 않습니다.'],
            ['이용 설정·번역 캐시', '자막 표시 옵션과 번역 결과는 이용자 브라우저(로컬)에만 저장되며 서버로 전송되지 않습니다.'],
          ]}
        />
        <p style={{ marginTop: 12 }}>확장 프로그램은 시청 기록을 수집·전송하지 않으며, 위 목적 외의 용도로 데이터를 사용하지 않습니다. 데이터를 제3자에게 판매하지 않습니다.</p>
      </Section>
