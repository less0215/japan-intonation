...
/* 플랜 업그레이드 화면 (/plans)
 * 핵심: "하루 N원"을 크게, 월/연 총액은 보조. 결제는 StoreKit/Play 연동 후 연결 — 지금은 안내 모달 */
const PLANS = {
  monthly: {
    pro:     { total: 4900,  per: 163, unit: '월' },
    premium: { total: 9900,  per: 330, unit: '월' },
    label: '월간',
  },
  yearly: {
    pro:     { total: 39000, per: 107, unit: '년', save: '2개월 무료' },
    premium: { total: 99000, per: 271, unit: '년', save: '2개월 무료' },
    label: '연간',
  },
}
...
export default function SubscriptionPage() {
  ...
  const [period, setPeriod] = useState('yearly')   // 기본 연간(하루 단가 더 낮게 보임)
  ...
  return (
    <>
      <PageSEO title="플랜 업그레이드 - 틱재팬" description="광고 제거 + 빠른 번역. 하루 163원부터." path="/plans" />
      ...
      <p style={{ margin: '0 2px 16px', fontSize: 13, color: 'var(--text-2)', lineHeight: 1.5 }}>커피 한 잔도 안 되는 값으로 광고를 없애고 빠른 번역을 펑펑 쓰세요.</p>
      ...
      <div style={{ position: 'relative', background: 'var(--surface)', border: `2px solid ${PRIMARY}`, borderRadius: 18, padding: '20px 18px 16px', marginBottom: 12 }}>
        <span style={{ position: 'absolute', top: -11, left: 18, background: PRIMARY, color: '#fff', fontSize: 11.5, fontWeight: 600, padding: '3px 12px', borderRadius: 8 }}>가장 인기 · 추천</span>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 2 }}>
          <span style={{ fontSize: 16, fontWeight: 700, color: PRIMARY }}>프로</span>
          <span style={{ fontSize: 11.5, color: 'var(--text-2)', background: 'var(--surface-2)', padding: '3px 9px', borderRadius: 7 }}>광고 제거 + 빠른번역</span>
        </div>
        <p style={{ margin: '12px 0 2px', fontSize: 31, fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text-strong)' }}>하루 {p.pro.per}원</p>
        <p style={{ margin: 0, fontSize: 13, color: 'var(--text-2)' }}>{p.pro.unit} ₩{p.pro.total.toLocaleString()}{p.pro.save && <span style={{ color: '#e0892a', marginLeft: 6 }}>· {p.pro.save}</span>}</p>
        <ul style={{ listStyle: 'none', padding: 0, margin: '14px 0 16px', display: 'flex', flexDirection: 'column', gap: 9 }}>
          {['광고 완전 제거', '빠른 번역 하루 200회 (사실상 무제한)', '모든 학습·라이브캠·저장'].map((f, i) => (
            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13.5, color: 'var(--text-1)' }}>
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke={PRIMARY} strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
              {f}
            </li>
          ))}
        </ul>
        <button onClick={() => choose('pro')} style={{ width: '100%', background: PRIMARY, color: '#fff', border: 'none', borderRadius: 13, padding: '14px', fontSize: 14.5, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>프로 시작하기 · 하루 {p.pro.per}원</button>
      </div>

      {/* 프리미엄 */}
      <div style={{ background: 'var(--surface)', border: '1px solid var(--bd)', borderRadius: 18, padding: '18px', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-strong)' }}>프리미엄</span>
          <span style={{ fontSize: 11.5, color: 'var(--text-2)', background: 'var(--surface-2)', padding: '3px 9px', borderRadius: 7 }}>무제한</span>
        </div>
        <p style={{ margin: '10px 0 2px', fontSize: 25, fontWeight: 800, letterSpacing: '-0.5px', color: 'var(--text-strong)' }}>하루 {p.premium.per}원</p>
        <p style={{ margin: 0, fontSize: 12.5, color: 'var(--text-2)' }}>{p.premium.unit} ₩{p.premium.total.toLocaleString()} · 번역가·업무 다량 사용자용</p>
        <ul style={{ listStyle: 'none', padding: 0, margin: '12px 0 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
          {['프로의 모든 혜택', '빠른 번역 무제한 (공정사용)', '신규 기능 우선 이용'].map((f, i) => (
            <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-2)' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#1d9e75" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
              {f}
            </li>
          ))}
        </ul>
        <button onClick={() => choose('premium')} style={{ width: '100%', background: 'var(--surface-2)', color: 'var(--text-1)', border: '1px solid var(--bd)', borderRadius: 13, padding: '13px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}>프리미엄 선택</button>
      </div>

      <p style={{ margin: '4px 2px 0', fontSize: 10.5, color: 'var(--text-3)', lineHeight: 1.6, textAlign: 'center' }}>
        구독은 App Store/Google Play 계정으로 결제·관리됩니다. 언제든 해지 가능.<br />빠른 번역 무제한은 공정사용 정책이 적용됩니다.
      </p>
      ...
      {notice && (
        <div onClick={() => setNotice(null)} style={{ position: 'fixed', inset: 0, zIndex: 4000, background: 'rgba(20,30,40,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: 300, maxWidth: '90vw', background: 'var(--surface)', borderRadius: 18, padding: '22px 20px 16px', textAlign: 'center', boxShadow: '0 16px 48px rgba(0,0,0,0.28)' }}>
            ...
            <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--text-2)', lineHeight: 1.6 }}>{notice === 'pro' ? '프로' : '프리미엄'} 구독은 출시 준비 중이에요.<br />오픈 알림을 보내드릴까요?</p>
