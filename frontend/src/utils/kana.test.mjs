...
import { KANA_HANGUL, moraToHangul, splitMora, kanjiGroupFlags, moraHangulLV, kataToHira, pronText } from './kana.mjs'
...
// 본문 발음(pronText) — LLM 발음 필드의 ざ→즈 오타 차단(결정론)
eq(pronText('はいざらをもらえますか', '灰皿(はいざら)をもらえますか'), '하이자라오모라에마스카', '灰皿 발음 ざ→자(즈 아님)')
eq(pronText('わたしはがくせいです', '私(わたし)は学生(がくせい)です'), '와타시와 가쿠세-데스', 'pronText 조사 は→와·장음·한자그룹 띄어쓰기')
eq(pronText('にほんごをべんきょうします', '日本語(にほんご)を勉強(べんきょう)します'), '니혼고오 벤쿄-시마스', 'pronText ん 받침·장음·を·띄어쓰기')
// 촉음 っ가 요음 뒤에 올 때 받침 결합(고아 ㅅ 방지): ちょっと→춋토
eq(pronText('ちょっとまってください', 'ちょっと待(ま)ってください'), '춋토 맛테쿠다사이', 'pronText 요음 뒤 촉음 ちょっと→춋토')
eq(pronText('しょっちゅうきます', 'しょっちゅう来(き)ます'), '숏츄우 키마스', 'pronText 요음 뒤 촉음 しょっ→숏')
