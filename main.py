...
Example — 日本語を話すことができますか？:
{
  "breakdown": [
    ...
    },
    {
      ...
      "part_of_speech": "동사",
      "conjugation_steps": [
        {"step": 1, "form": "話す", "reading": "はなす", "label": "기본형 (사전형)", "note": "1그룹(5단) 동사예요."}
      ]
    },
    {
      "unit": "ことができますか",
      ...
      "conjugation_steps": [
        {"step": 1, "form": "できる", "reading": "できる", "label": "기본형", "note": "가능·잠재를 나타내는 2그룹(1단) 동사예요."},
        {"step": 2, "form": "できます", "reading": "できます", "label": "ます형 (정중체)", "note": "できる를 정중하게 바꾼 형태예요(できる→できます)."},
        {"step": 3, "form": "できますか", "reading": "できますか", "label": "의문형", "note": "끝에 か를 붙이면 정중한 의문문이 돼요."}
      ],
...
- "conjugation_steps": null for uninflected chunks; array for conjugated/pattern chunks
  Each step: {"step": <int>, "form": <Japanese>, "reading": <hiragana reading of form, for furigana display>, "label": <Korean label>, "note": <Korean explanation>}
  ...
  동사 그룹: 1그룹(5단)=어미가 う단(う·く·ぐ·す·つ·ぬ·ぶ·む·る) / 2그룹(1단)=い단·え단 소리 + る(出る·食べる·見る·寝る·着る·起きる·受ける·続ける·教える·覚える·考える) / 3그룹(불규칙)=する·来る(くる).
  ⚠️ 그룹 판정 흔한 오류 — 끝이 〜いる/〜える라도 **1그룹인 예외 동사**가 있어요: 帰る(かえる)·入る(はいる)·走る(はしる)·要る(いる)·知る(しる)·切る(きる)·喋る(しゃべる)·焦る(あせる)·握る(にぎる)·蹴る(ける)·滑る(すべる)·散る(ちる)·限る(かぎる)·混じる(まじる)·参る(まいる) → 부정형이 〜らない(帰る→帰らない). 반대로 出る·食べる·見る·寝る·着る·起きる·受ける 등은 **진짜 2그룹**(부정 出る→出ない). 이 둘을 절대 혼동하지 마세요. (出る는 2그룹입니다.)
  ⚠️ 자기검증(필수) — 1단계에서 밝힌 그룹과 뒤 단계의 변형 규칙이 반드시 일치해야 해요. 출력 전 점검: 부정/て형에서 **る만 떼고 ない/て**가 붙었으면 2그룹(出る→出ない), **끝모음을 あ/い/え단으로 바꿔** 붙었으면 1그룹(飲む→飲まない). 라벨과 실제 변형이 어긋나면 1단계 그룹 라벨을 결과에 맞게 고치세요. 〜なる·〜がる로 끝나는 보조동사(なくなる의 なる 등)는 1그룹입니다.
  ⚠️ 음편(音便) 자기검증(필수) — て/た형을 쓸 때 1그룹 어미별 음편표를 아래 그대로 적용하세요. 절대 임의로 다른 글자를 붙이지 마세요: う·つ·る→って/った(買う→買って·勝つ→勝って·帰る→帰って) / ぬ·ぶ·む→んで/んだ(飲む→飲んで·呼ぶ→呼んで·死ぬ→死んで) / く→いて/いた(書く→書いて·結びつく→結びついて) / ぐ→いで/いだ(泳ぐ→泳いで) / す→して/した(話す→話して), 예외 行く→行って/行った. 다른 동사(예: 結び付ける는 別語)와 헷갈리지 마세요 — 어미 く로 끝나는 1그룹 동사는 반드시 い+て/た가 붙어요.
  ※ 아래 표에서 **이 chunk에 실제로 나온 형태만** 골라 설명하세요(전부 나열 금지). 규칙은 정확히 적용:
   ...
   · た형(과거·반말체): て형과 같은 음편에서 て→た·で→だ (買った·飲んだ·書いた·話した / 食べた / した·来た)
   · ました형(과거·정중체) ⚠️흔한 오류 주의: **ます형에서 ます를 떼고 +ました**를 붙여요(飲みます→飲みました, 食べます→食べました, します→しました, 来ます→来ました(きました)). て형(飲んで·食べて)에 ます를 붙이는 게 아니에요 — て형과 ました형은 서로 다른 경로예요. 절대 "て형에 ます를 붙여서" 같은 설명을 쓰지 마세요.
...
- "note": a SHORT, beginner-friendly Korean explanation. Assume the learner JUST started — can read hiragana but knows almost no grammar. The goal is not "the form changed" but "you can actually USE this." Fill note whenever the chunk is NOT self-evident:
    ...
    • 명사+조사 chunk에서 조사가 단순 주격(が)·목적격(を)·주제(は)가 **아니라** 여러 뜻으로 번역될 수 있는 조사(で·に·から·まで·と·や·の·へ 등)라면, note에 "이 조사가 왜 이 뜻인지" 1문장으로 짚어주세요. 초보자는 が/を/は는 직관적으로 알아도 で/に의 다양한 용법은 헷갈려요. 예: 銃で → "で는 수단·도구를 나타내는 조사예요(~으로, ~을 가지고). 총이라는 도구를 썼다는 뜻이에요." 예: 三時に → "に는 시간·시점을 나타내는 조사예요(~에)."
    • 가능형 동사(〜できる·가능형 られる/れる 등)와 함께 쓰인 명사의 조사가 **を가 아니라 が**인 경우(예: 演奏ができる = 연주를 할 수 있다) → note에 "가능을 나타낼 때는 목적어에 조사 を 대신 が를 쓰는 경우가 많아요(을/를 자리에 が)"처럼 짚어주세요.
...
# v8: 동사 그룹 오분류 방지 — 예외 1그룹(帰る·入る류) 목록 + 진짜 2그룹(出る류) 명시 + 라벨↔변형 자기검증(出る=2그룹)
# v9: ました형(과거·정중체) 누락 규칙 추가(て형+ます로 잘못 설명하던 오류 수정) + 음편(く→いた 등) 자기검증 강화
#     + conjugation_steps에 reading(후리가나용) 추가 + 조사(で·に 등) 의미·が/を 가능형 교체 설명 강화
_BD_CACHE_VER = "v9"
...
class Subscription(Base):
    """유료 구독 — iOS 인앱결제(RevenueCat) 및 관리자 지급. User/SavedResult 스키마와 무관한 별도 테이블.
    ...
    billing_key = Column(Text, nullable=True)            # 레거시 빌링키(미사용 — IAP는 RevenueCat 처리)
...
class ConjugationStep(BaseModel):
    ...
    label: str
    note: str
    reading: str | None = None   # form의 히라가나 읽기(후리가나 표시용)
...
# ── 구독 플랜 기간(일) — admin/grant-sub·서버 enforcement에서 기간 산정에 사용 ──
# 결제 자체는 iOS 인앱결제(RevenueCat)로 일원화됨. 금액은 표시/기록용(실청구는 App Store).
# 플랜·주기별 금액(원)·기간(일)
SUB_PRICES = {
    ("plus", "monthly"): (4400,   30),
    ("plus", "yearly"):  (44000,  365),
    ("pro",  "monthly"): (19000,  30),
    ("pro",  "yearly"):  (190000, 365),
}
...
# 웹 토스 결제 제거 — 구독은 iOS 인앱결제(RevenueCat)로 일원화됨(/revenuecat/webhook).
